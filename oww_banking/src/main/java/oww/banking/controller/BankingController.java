...
public class BankingController {
	...
	/**
	 * 계좌 정보 API (게이트웨이 헤더 기반)
	 */
	@GetMapping(value = "/api/account", produces = "application/json")
	@ResponseBody
	public ResponseEntity<Map<String, Object>> getAccountInfoApi(Authentication authentication, HttpServletRequest request) {
	...
	/**
	 * 계좌 생성 페이지 (게이트웨이 헤더 기반)
	 */
	@GetMapping("/banking/createAccount")
	public String createAccount(Model model, Authentication authentication, HttpServletRequest request) {
		System.out.println("계좌 생성 페이지 호출됨");

		if (authentication == null || !authentication.isAuthenticated()) {
			System.out.println("인증 실패 - 메인으로 리다이렉트");
			return "redirect:http://localhost:8201/main";
		}
	...
	/**
	 * 계좌 생성 성공 페이지 (게이트웨이 헤더 기반)
	 */
	@GetMapping("/banking/account/success")
	public String accountSuccess(Model model, Authentication authentication, HttpServletRequest request) {
	    ...
	    try {
	        ...
	        // 이메일로 계좌 조회
	        AccountVO newAccount = accountService.getAccountByEmail(userEmail);
	...
	public String bankingMain(Model model, Authentication authentication, HttpServletRequest request) {
	    ...
	    if (authentication == null || !authentication.isAuthenticated()) {
	        return "redirect:http://localhost:8201/"; // 인증되지 않은 경우 메인으로
	    }
	...
	public ResponseEntity<String> health() {
		return ResponseEntity.ok("Banking Service is running on port 8203 - Gateway Header Based");
	}
}