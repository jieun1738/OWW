...
public class BankingSecurityConfig implements WebMvcConfigurer {
    ...
    public static class TrustedGatewayFilter extends OncePerRequestFilter {
        ...
        protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
            ...
            log.info("🔍 TrustedGatewayFilter 진입: {}", requestURI);
            ...
            if (SecurityContextHolder.getContext().getAuthentication() != null) {
                log.info("✅ 이미 인증된 SecurityContext 발견, 통과");
                filterChain.doFilter(request, response);
                return;
            }
            ...
            // 임시로 Gateway 헤더가 없어도 JWT 필터에서 처리하도록 일단 통과시킴 (디버깅용)
            log.warn("⚠️ Gateway 헤더 부족하지만 JWT 필터에서 처리할 수 있도록 통과: {}", requestURI);
            filterChain.doFilter(request, response);
        }
