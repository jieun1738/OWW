...
public class EmailService {
    ...
    public void sendVerificationEmail(String toEmail, String verificationCode, String userName) {
        try {
            // ✅ 디버깅 로그 추가 - 특별한 키워드로 구분
            System.out.println("🚨🚨🚨 EMAIL_DEBUG_START 🚨🚨🚨");
            System.out.println("EMAIL_DEBUG: mailSender 상태: " + (mailSender != null ? "정상" : "NULL"));
            System.out.println("EMAIL_DEBUG: templateEngine 상태: " + (templateEngine != null ? "정상" : "NULL"));
            System.out.println("EMAIL_DEBUG: fromEmail: " + fromEmail);
            System.out.println("EMAIL_DEBUG: fromName: " + fromName);
            System.out.println("EMAIL_DEBUG: toEmail: " + toEmail);
            System.out.println("EMAIL_DEBUG: verificationCode: " + verificationCode);
            System.out.println("🚨🚨🚨 EMAIL_DEBUG_CONFIG_END 🚨🚨🚨");

            MimeMessage message = mailSender.createMimeMessage();
            System.out.println("EMAIL_DEBUG: MimeMessage 생성 성공");
            
            MimeMessageHelper helper = new MimeMessageHelper(message, true, "UTF-8");
            System.out.println("EMAIL_DEBUG: MimeMessageHelper 생성 성공");

            helper.setFrom(fromEmail, fromName);
            System.out.println("EMAIL_DEBUG: From 설정 완료");
            
            helper.setTo(toEmail);
            System.out.println("EMAIL_DEBUG: To 설정 완료");
            
            helper.setSubject("[Own Wedding Wallet] 계좌 개설 이메일 인증");
            System.out.println("EMAIL_DEBUG: Subject 설정 완료");

            // HTML 템플릿 사용
            String htmlContent = createVerificationEmailContent(verificationCode, userName);
            System.out.println("EMAIL_DEBUG: HTML 콘텐츠 생성 완료");
            
            helper.setText(htmlContent, true);
            System.out.println("EMAIL_DEBUG: 메일 내용 설정 완료");

            System.out.println("🚨🚨🚨 EMAIL_SEND_START 🚨🚨🚨");
            mailSender.send(message);
            System.out.println("🚨🚨🚨 EMAIL_SEND_SUCCESS 🚨🚨🚨");
            System.out.println("EMAIL_DEBUG: 이메일 발송 성공: " + toEmail);

        } catch (Exception e) {
            System.out.println("🚨🚨🚨 EMAIL_ERROR_START 🚨🚨🚨");
            System.out.println("EMAIL_ERROR: 오류 타입: " + e.getClass().getSimpleName());
            System.out.println("EMAIL_ERROR: 오류 메시지: " + e.getMessage());
            System.out.println("🚨🚨🚨 EMAIL_ERROR_END 🚨🚨🚨");
            e.printStackTrace();
