...
public class AccountService {
    ...
    public Map<String, Object> login(String email, String password) {
        ...
        try {
            ...
            if (user != null && account != null && account.getAccountPassword().equals(password)) {
                String token = jwtUtil.generateToken((long)user.getUserNo(), user.getName(), user.getRole(), user.getUserEmail());
    ...
    public String createAccount(String name, String email, String password, String emailCode) {
        try {
            log.info("🏦 계좌 생성 시작: {}", email);
            
            // 1. 이메일 인증번호 확인
            if (!verifyEmailCode(email, emailCode)) {
                log.warn("❌ 이메일 인증번호 불일치: {}", email);
                return "이메일 인증번호가 올바르지 않습니다.";
            }

            // 2. 이메일로 이미 계좌가 존재하는지 확인
            if (accountMapper.existsByEmail(email)) {
                log.warn("❌ 이미 존재하는 계좌 (이메일): {}", email);
            ...
            if (user == null) {
                ...
                user.setActive(true);
                user.setRole("USER");
                user.setProvider("LOCAL"); // 기본 제공자
                userMapper.createUser(user);
                
                // 생성된 사용자 정보 다시 조회 (userNo 가져오기 위해)
                user = userMapper.findByEmail(email);
                if (user == null) {
                    log.error("❌ 사용자 생성 후 조회 실패: {}", email);
                    return "사용자 생성에 실패했습니다.";
                }
            }

            // 4. ✨ 1계정 1계좌 제한 - 이메일로 계좌 존재 여부 재확인 (이미 위에서 체크했지만 안전장치)
            // 이미 위에서 existsByEmail로 체크했으므로 이 부분은 추가 안전장치입니다.

            // 5. 실제 은행 스타일 계좌번호 생성
            String accountNumber = generateBankStyleAccountNumber();
            log.info("🔢 생성된 계좌번호: {}", accountNumber);

            // 6. 계좌 객체 생성
            AccountVO account = new AccountVO();
            account.setUserEmail(email); // 이메일 설정
            ...
            if (result > 0) {
                ...
                log.info("✅ 계좌 생성 성공: {}", accountNumber);
            ...
            log.error("❌ 계좌 생성 중 오류: {}", e.getMessage(), e);
    ...
    public String sendEmailVerification(String email) {
        try {
            ...
            if (emailService == null) {
                log.error("❌ EmailService가 주입되지 않았습니다!");
                throw new RuntimeException("EmailService가 초기화되지 않았습니다.");
            }
            
            // 5. 실제 이메일 발송
            log.info("📧 EmailService.sendVerificationEmail 호출 중...");
            ...
            log.error("❌ 이메일 발송 실패: {} - {}", e.getClass().getSimpleName(), e.getMessage(), e);
    ...
    public boolean verifyEmailCode(String email, String inputCode) {
        ...
        log.info("🔍 이메일 인증번호 확인: {} - {}", email, isValid ? "성공" : "실패");
        return isValid;
    }
