...
public class BankingJwtUtil {
    ...
    // 토큰 생성
    public String generateToken(Long userNo, String username, String role, String userEmail) {
        return Jwts.builder()
            .claim("userNo", userNo)
            .claim("role", role)
            .claim("userEmail", userEmail) // 이메일 클레임 추가
            .setSubject(username)
 ...
 // 이메일 추출
    public String extractUserEmail(String token) {
        Claims claims = extractAllClaims(token);
        return claims.get("userEmail", String.class); // userEmail로 통일
    }
