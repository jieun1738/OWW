...
public class JwtUtil {
    ...
    /**
     * 토큰에서 사용자명 추출 (Login Service와 일치)
     */
    ...
    /**
     * 토큰에서 역할(role) 추출 (Login Service와 일치)
     */
    public String extractRole(String token) {
        try {
            String role = extractClaim(token, claims -> claims.get("role", String.class));
            System.out.println("추출된 역할: " + role);
            return role != null ? role : "USER"; // 기본값 설정
        } catch (Exception e) {
    ...
    /**
     * 토큰에서 사용자 번호 추출 (Login Service와 일치)
     */
    ...
    public TokenValidationResult validateTokenWithDetails(String token) {
        try {
            ...
            String userEmail = extractUserEmail(token);  // extractUserEmail로 변경

            return new TokenValidationResult(true, "토큰 검증 성공", username, userNo, role, userEmail);
    ...
    public static class TokenValidationResult {
        ...
        public TokenValidationResult(boolean valid, String message, String username, String userNo, String role, String userEmail) {
            this.valid = valid;
        ...
        public String getUserEmail() { return userEmail; }  // getUserEmail로 변경
    }
}